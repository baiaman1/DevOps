...
int main(int argc, char *argv[]) {
  ...
  openFile(argc, argv, opt, pattern);
...
void openFile(int argc, char *argv[], options opt, char pattern[]) {
  ...
  for (int i = optind; i < argc; i++) {
    ...
    if (file) {
      ...
      if (opt.l)
        nameFile = argv[i];
      printLine(file, pattern, opt, nameFile);
      fclose(file);
...
int findPat(char *tmpLine, char pattern[], options *opt) {
  ...
  regcomp(&regex, pattern, regFlag); // i
  if (opt->v)
    res = REG_NOMATCH; // v
...
void printLine(FILE *file, char pattern[], options opt, char *nameFile) {
  ...
  while (ext) {
    ...
    int success = findPat(tmp_line, pattern, &opt);
    if (line == NULL) {
      ext = 0;
    } else if (success == 0) {
      opt.count++;
...
char *s21_strcat(char *dest, const char *src) {
  // Almost always, snprintf is better than strcat aboba
  char *flag;
  char *start = dest;
  while (*start != '\0') {
    start++;
